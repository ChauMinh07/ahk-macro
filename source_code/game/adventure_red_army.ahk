...
;Get Config
adventure_red_army_config() {
    global name_config := "adventure_red_army"
...
setting_GUI_adventure_red_army() {
    ...
    ChangeButton.OnEvent('Click', ChangeSettingsEvent_adventure_red_army)

    act_settings_GUI.Show("w495 h350")
}

ChangeSettingsEvent_adventure_red_army(*) {
    ...
    if (totalPlace["Ground"] > 0) {
        try {
            adventure_red_army_getGroundPos(totalPlace["Ground"])
        } catch Error {
    ...
    if totalPlace["Hill"] > 0 {
        try {
            adventure_red_army_getHillPos(totalPlace["Hill"])
        } catch Error {
    ...
    if totalPlace["Farm"] > 0 {
        try {
            adventure_red_army_getFarmPos(totalPlace["Farm"])
        } catch Error {
...
starting_adventure_red_army() {
    if (WinExist("ahk_exe RobloxPlayerBeta.exe")) {
        ...
        if (WinActive("ahk_exe RobloxPlayerBeta.exe")) {
            ...
            ;if !ImageSearch(&FoundX, &FoundY, 0, 0, W, H, adventure_red_army) {
...
starting_adventure_red_army_ingame() {
    if (WinExist("ahk_exe RobloxPlayerBeta.exe")) {
        ...
        if (WinActive("ahk_exe RobloxPlayerBeta.exe")) {
            ...
            Sleep(750)
            if (ImageSearch(&FoundX, &FoundY, 0, 0, W, H, wrongspawn_adventure_red_army) || ImageSearch(&FoundX, &
                FoundY, 0, 0, W, H, wrongspawn_adventure_red_army_2)) {
                ...
                ;Tele to spawn
                while !ImageSearch(&FoundX, &FoundY, 505, 0, 585, 60, adventure_red_army_spawn_condition) {
...
adventure_red_army_duringGame() {
    ...
    SetTimer adventure_red_army_chooseScreen, 100
    ...
    if (placed <= Slot1Place) {
        while (placed <= Slot1Place) {
            while !ImageSearch(&FoundX, &FoundY, 0, 0, W, H, upgrade_ui) {
                if Slot1Type == "Ground" {
                    getPos := adventure_red_army_getGroundPos(groundPlaced + 1)
                    xy := StrSplit(getPos, ",")
                    x_place := xy[1]
                    y_place := xy[2]
                } else if (Slot1Type == "Hill") {
                    getPos := adventure_red_army_getHillPos(hillPlaced + 1)
                    xy := StrSplit(getPos, ",")
                    x_place := xy[1]
                    y_place := xy[2]
                } else if (Slot1Type == "Farm") {
                    getPos := adventure_red_army_getFarmPos(farmPlaced + 1)
                    xy := StrSplit(getPos, ",")
                    x_place := xy[1]
                    y_place := xy[2]
                }
...
adventure_red_army_retryGame() {
    ...
    SetTimer adventure_red_army_chooseScreen, 0
...
adventure_red_army_chooseScreen() {
    choosescreen :=
...
adventure_red_army_getFarmPos(index) {
...
adventure_red_army_getHillPos(index) {
    ;x,y
    HillPos := Map()
...
adventure_red_army_getGroundPos(index) {
    ;x,y
    GroundPos := Map()
